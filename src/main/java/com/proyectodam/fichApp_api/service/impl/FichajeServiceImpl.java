...
public class FichajeServiceImpl implements IFichajeService {
    ...
    public FichajeServiceImpl(FichajeRepository fichajeRepository, EmpleadoRepository empleadoRepository,
                               ContratoRepository contratoRepository, BolsaHorasRepository bolsaHorasRepository) {
    ...
    public Fichaje registrarFichaje(FichajeRequestDTO fichajeRequestDTO) {
        Empleado empleado = empleadoRepository.findById(fichajeRequestDTO.getIdEmpleado())
        ...
        Fichaje savedFichaje = fichajeRepository.save(fichaje);
    ...
    private EstadoJornada calcularEstadoActualEmpleado(int idEmpleado) {
        Optional<Fichaje> ultimoFichaje = fichajeRepository
                .findTopByEmpleadoIdEmpleadoOrderByTimestampServidorDesc(idEmpleado);

        if (ultimoFichaje.isEmpty()) {
            return EstadoJornada.FUERA;
        }

        return switch (ultimoFichaje.get().getTipoEvento()) {
    ...
    private void validarTransicion(EstadoJornada estadoJornada, TipoEventoFichaje tipoEventoFichaje) {
        boolean valido = switch (estadoJornada) {
            ...
            case DENTRO -> tipoEventoFichaje == TipoEventoFichaje.PAUSA_INICIO
                    || tipoEventoFichaje == TipoEventoFichaje.SALIDA;
            case EN_PAUSA -> tipoEventoFichaje == TipoEventoFichaje.PAUSA_FIN;
        };

        if (!valido) {
            throw new RuntimeException("Transición de fichaje inválida. Estado actual: "
                    + estadoJornada + " | Evento recibido: " + tipoEventoFichaje);
        }
    }

    private void validarHorario(Empleado empleado) {
        LocalDate hoy = LocalDate.now();
        Contrato contrato = contratoRepository.buscarContratoActivo(empleado.getIdEmpleado(), hoy)
                .orElseThrow(() -> new RuntimeException("Empleado sin contrato activo"));
    ...
    public List<Fichaje> obtenerFichajesHoy(int idEmpleado) {
        LocalDate hoy = LocalDate.now();
        return fichajeRepository.findByEmpleadoIdEmpleadoAndTimestampServidorBetween(
                idEmpleado, hoy.atStartOfDay(), hoy.atTime(LocalTime.MAX));
    }
    ...
    public ContadorDTO obtenerContador(int idEmpleado) {
        ...
        LocalDate hoy = LocalDate.now();
        List<Fichaje> fichajesHoy = fichajeRepository
                .findByEmpleadoIdEmpleadoAndTimestampServidorBetween(
                        idEmpleado, hoy.atStartOfDay(), hoy.atTime(LocalTime.MAX))
                .stream().sorted(Comparator.comparing(Fichaje::getTimestampServidor)).toList();
        ...
        for (Fichaje f : fichajesHoy) {
            switch (f.getTipoEvento()) {
                case ENTRADA, PAUSA_FIN:
                    if (ultimaEntrada == null) ultimaEntrada = f.getTimestampServidor();
                    break;
                case SALIDA, PAUSA_INICIO:
                    if (ultimaEntrada != null && f.getTimestampServidor().isAfter(ultimaEntrada)) {
                        segundos += Duration.between(ultimaEntrada, f.getTimestampServidor()).getSeconds();
                        ultimaEntrada = null;
                    }
                    break;
            }
        ...
        if (estadoJornada == EstadoJornada.DENTRO && ultimaEntrada != null) {
            ...
            if (ahora.isAfter(ultimaEntrada)) {
                segundos += Duration.between(ultimaEntrada, ahora).getSeconds();
            }
        }
    ...
    public HorasExtraDTO calcularHorasExtra(int idEmpleado) {
        ...
        Optional<Contrato> contratoOpt = contratoRepository.buscarContratoActivo(idEmpleado, hoy);
        /* Contrato contrato = contratoRepository.buscarContratoActivo(idEmpleado, hoy)
                .orElseThrow(() -> new RuntimeException("Contrato no encontrado")); */
        
        if (contratoOpt.isEmpty()) {
            return new HorasExtraDTO(horasTrabajadas, 0.0, horasTrabajadas, obtenerSaldoActual(idEmpleado));
        }
    ...
    private double obtenerSaldoActual(int idEmpleado) {
        ...
        return bolsaHorasRepository.findByEmpleadoIdEmpleadoAndAnio(idEmpleado, anio)
                .map(bh -> bh.getSaldoHoras() != null ? bh.getSaldoHoras() : 0.0)
                .orElse(0.0);
    }
    ...
    private long calcularSegundosTrabajados(List<Fichaje> fichajes, boolean incluirTiempoActual) {
        ...
        for (Fichaje fichaje : fichajes) {
            switch (fichaje.getTipoEvento()) {
                case ENTRADA, PAUSA_FIN:
                    if (inicio == null) inicio = fichaje.getTimestampServidor();
                    break;
                case PAUSA_INICIO, SALIDA:
                    if (inicio != null) {
    ...
    public Contrato obtenerContratoActivo(int idEmpleado, LocalDate fecha) {
        return contratoRepository.buscarContratoActivo(idEmpleado, fecha)
                .orElseThrow(() -> new RuntimeException("Empleado sin contrato activo"));
    }
    ...
    public double recalcularBolsaHoras(int idEmpleado) {
        ...
        List<Fichaje> fichajes = fichajeRepository
                .findByEmpleadoIdEmpleadoAndTimestampServidorBetween(
                        idEmpleado, inicioAnio.atStartOfDay(), hoy.atTime(LocalTime.MAX))
                .stream().sorted(Comparator.comparing(Fichaje::getTimestampServidor)).toList();

        if (fichajes.isEmpty()) return 0.0;

        Map<LocalDate, List<Fichaje>> fichajePorDia = fichajes.stream()
                .collect(Collectors.groupingBy(
                        f -> f.getTimestampServidor().toLocalDate(),
                        TreeMap::new,
                        Collectors.toList()));
        ...
        for (Map.Entry<LocalDate, List<Fichaje>> entry : fichajePorDia.entrySet()) {
            ...
            List<Fichaje> fichajesDia = entry.getValue().stream()
                    .sorted(Comparator.comparing(Fichaje::getTimestampServidor)).toList();

            if (fichajesDia.isEmpty()) continue;

            boolean tieneEntrada = fichajesDia.stream()
                    .anyMatch(f -> f.getTipoEvento() == TipoEventoFichaje.ENTRADA);
            if (!tieneEntrada) continue;

            long segundos = calcularSegundosTrabajados(fichajesDia, false);
            double horasTrabajadas = segundos / 3600.0;

            Optional<Contrato> contratoOpt = contratoRepository.buscarContratoActivo(idEmpleado, fecha);
            /* Contrato contrato = contratoRepository.buscarContratoActivo(idEmpleado, fecha)
                    .orElseThrow(() -> new RuntimeException("Contrato no encontrado")); */
            
            if (contratoOpt.isEmpty()) {
                System.out.println("ADVERTENCIA: No se encontró contrato para el empleado " + idEmpleado + " en la fecha " + fecha + ". Saltando día en el cálculo de bolsa.");
                continue;
            }
            Contrato contrato = contratoOpt.get();
            double horasContrato = contrato.getHorasDiarias() != null ? contrato.getHorasDiarias() : 8.0;

            boolean jornadaTerminada = fichajesDia.stream()
                    .anyMatch(f -> f.getTipoEvento() == TipoEventoFichaje.SALIDA);

            if (jornadaTerminada) {
                saldo += horasTrabajadas - horasContrato;
            }
        }

        Empleado empleado = empleadoRepository.findById(idEmpleado).orElseThrow();
        BolsaHoras bolsaHoras = bolsaHorasRepository.findByEmpleadoIdEmpleadoAndAnio(idEmpleado, anio)
                .orElseGet(() -> {
